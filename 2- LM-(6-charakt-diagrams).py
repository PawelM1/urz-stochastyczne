...
############
    ...
    def diagram( self ):
        #graf = nx.DiGraph( [('a', 'b'), ('a', 'c'), ('b', 'a'),
        #                    ('c', 'b'), ('c', 'c')] )
        grafPre = []
        for i in range(len(stany)):
            for j in range(len(stany)):
                if self.macPP[i][j] > 0:
                    grafPre.append( (stany[i], stany[j]) )
        graf = nx.DiGraph( grafPre )
        ...
        for i in range(1,10):
            for j in range(1,10):
                nodePosPre.append( np.array([i*0.1, j*0.1]) )
        node_positions, used = [], []
        for i in range(len(stany)):
            pos = npr.choice( nodePosPre )
            while pos not in used:
                node_positions[ stany[i] ] = nodePosPre[ pos ]
                pos = npr.choice( nodePosPre )
        ...
        edge_labels = {}
        for gr in grafPre:
            edge_labels[ gr ] = self.macPP[
                self.slownikStanow[ gr[0] ],
                self.slownikStanow[ gr[1] ]
                ]
            

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14,14))

        Graph(graf, node_labels=True, edge_labels=edge_labels,
